...
export async function materielNetOffers({ q, ean }) {
  const search = ean || q;
  if (!search) return [];
  return withBrowser(async (page) => {
    await page.goto(`https://www.materiel.net/recherche/${encodeURIComponent(search)}/`, { waitUntil: 'domcontentloaded' });
    const first = page.locator('a.product-item__title, a.product__title').first();
    if (!await first.count()) return [];
    const href = await first.getAttribute('href');
    if (!href) return [];
    const url = new URL(href, 'https://www.materiel.net').toString();
    ...
    if (!price) {
      const ptxt = await page.locator('.price, .product-price__price, .c-product-price__current').first().innerText().catch(()=>'');
    ...
    return [{
      productId: search, vendor: 'materiel-net', price, currency: 'EUR', inStock,
