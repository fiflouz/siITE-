...
export async function dartyOffers({ q, ean }) {
  const search = ean || q;
  if (!search) return [];
  return withBrowser(async (page) => {
    await page.goto(`https://www.darty.com/nav/recherche?text=${encodeURIComponent(search)}`, { waitUntil: 'domcontentloaded' });
    const first = page.locator('a[data-analytics-click*="product"]').first();
    if (!await first.count()) return [];
    const href = await first.getAttribute('href');
    if (!href) return [];
    const url = href.startsWith('http') ? href : `https://www.darty.com${href}`;
    ...
    if (!price) {
      const ptxt = await page.locator('.darty_prix, .price, .ds_price').first().innerText().catch(()=>'');
    ...
    return [{
      productId: search, vendor: 'darty', price, currency: 'EUR', inStock,
