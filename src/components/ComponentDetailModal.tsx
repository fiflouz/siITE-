...
export const ComponentDetailModal = ({ component, onClose, priceMap }: ComponentDetailModalProps) => {
  ...
  const getCategoryIcon = (componentName: string) => {
    if (componentName.includes("Intel Core") || componentName.includes("Ryzen")) return <Cpu className="w-8 h-8 text-[#4A90E2]" />;
    if (componentName.includes("RTX") || componentName.includes("Radeon")) return <Monitor className="w-8 h-8 text-[#4A90E2]" />;
    if (componentName.includes("ASUS") || componentName.includes("MSI") || componentName.includes("Gigabyte") || componentName.includes("ASRock")) {
      if (component.specs.some(s => s.includes("LGA") || s.includes("AM"))) return <Cpu className="w-8 h-8 text-[#4A90E2]" />; // Motherboard
    }
    if (componentName.includes("DDR")) return <MemoryStick className="w-8 h-8 text-[#4A90E2]" />;
    if (componentName.includes("SSD") || componentName.includes("HDD")) return <HardDrive className="w-8 h-8 text-[#4A90E2]" />;
    if (componentName.includes("W") && component.specs.some(s => s.includes("80+"))) return <Zap className="w-8 h-8 text-[#4A90E2]" />;
    return <Info className="w-8 h-8 text-[#4A90E2]" />;
  };
          ...
          >
            ...
            <div className="flex items-center gap-6 mb-6">
              ...
              <div>
                ...
                <div className="text-2xl font-bold text-[#4A90E2] mt-2">{fmt(displayPrice)}</div>
                {externalPriceInfo && (
                  <div className="text-xs text-[#A1A1AA] mt-1">
                    via <a className="underline text-[#4A90E2] hover:text-[#5BA3F5]" href={externalPriceInfo.url} target="_blank" rel="noopener noreferrer">
                ...
                <ul className="space-y-1 text-[#A1A1AA] text-sm">
                  {component.specs.map((spec, index) => (
                    <li key={index} className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 bg-[#4A90E2] rounded-full flex-shrink-0" />
                      <span>{spec}</span>
                    </li>
                  ))}
