...
export const Layout = ({ children }: LayoutProps) => {
  ...
  const { isLoggedIn, openAuthModal, closeAuthModal, authModalOpen } = useAuth();
      ...
      <div className="fixed top-8 left-0 right-0 z-50 flex justify-center">
        ...
        >
          ...
          <Link to="/" className="flex items-center gap-3">
            ...
            <span className="text-xl font-bold text-[#F5F5F7] tracking-tight">PC Builder</span>
          </Link>
            ...
            {navigationItems.map((item) => (
              ...
              >
                <span className="relative inline-flex items-center">
                  {item.name}
                  {item.name === "Comparateur" && compareCount > 0 && (
                    <span className="absolute -top-2 -right-6 flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-gradient-to-r from-[#4A90E2] to-[#5BA3F5] rounded-full">
                      {compareCount}
                    </span>
                  )}
                </span>
